...
const projects = ["pg", "pg.client", "pg.hosting.client", "bda"];
...
const getParsedTime = (t) =>
  String(new Date(t + TIME_OFFSET).getHours()).padStart(2, "0") +
  ":" +
  String(new Date(t + TIME_OFFSET).getMinutes()).padStart(2, "0");
const getEnvStatus = (p, e) =>
  `🧪 *${e}*: ${
    store[p][e].busy
      ? `🔒 Locked by ${store[p][e].user} until ${getParsedTime(
          store[p][e].timestamp
        )} - (${getMinutesRemaining(store[p][e].timestamp)} minutes left)`
      : "🔓 Free"
  }`;
...
const getCurrentStatus = () =>
  ...
  "‼️ Current status ‼️" +
  separator +
  projects.map(getProjectStatus).join(separator);
...
const updateAllStates = async () => {
  ...
  const emptyStore = projects.reduce(
    (pAcc, pCurr) => ({
      ...pAcc,
      [pCurr]: envs.reduce(
        (eAcc, eCurr) => ({ ...eAcc, [eCurr]: getEmptyState() }),
        {}
      ),
    }),
    {}
  ...
  store = { ...emptyStore, ...store }; // USED TO ADD NEW PROJECTS IF NOT EXISTING IN DB
...
const lock = async (argString, user_name, say) => {
  ...
  if (current.busy && current.user !== user_name) {
    await say(
      `❌🔒 Sorry, ${_project}/${_env} is currently locked by @${
        current.user
      } until ${getParsedTime(
        current.timestamp
      )}. Try again later or ask them to unlock.`
    );
    return;
  }
  ...
  await say(
    `✅🔒 ${_project}/${_env} is now *locked* by @${
      current.user
    } until ${getParsedTime(current.timestamp)}`
  );
  await saveToDB();
...
app.command("/unlock", async ({ command, ack, say }) => {
  ...
  if (current.user !== user_name) {
    await say(
      `❌ This project/env is not locked by you ${user_name}, ask ${current.user} to unlock it`
    );
    return;
  }
